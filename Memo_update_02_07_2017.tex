...
\end{itemize}
 	 ...
 	 \subsection{Formules de préférence contextuelle}
 	 Dans notre travail, nous allons nous inspiré de la représentation en formules de préférences décrite précédamment pour l'appliquer au cas des règles de préférence contextuelles.
 	 Nous avions décrit précédamment la représentation des formules de préférences comme suit:
 	 	 \[  
 	 	 	C(t_{1},t_{2})=\underset{i=1..k}{\bigvee}(\underset{j=1..l}{\bigwedge} f_{ij})
 	 	 \]
 	Afin d'appliquer cette formulation à notre cas de règles de préférences contextuelles, nous faisons le choix d'utiliser les formules de préférence comme étant des formules de préférence basées seulement sur les connecteurs logiques $\wedge$. Ainsi une formule logique $P(t_1,t_2)$ suivant notre approche sera sous la forme: 
 	     \begin{equation}
 	     	 P(t_{1},t_{2})=\underset{k\in E}{\bigwedge} P_{A_{k}}(t_{1},t_{2})
 	     \end{equation}
 	     	
     
      avec $E\subset \{1,2,..,n\}$ où n est le nombre des différents types d'attributs $A_k$ qui définissent les items des transactions $t_1$, $t_2$ et $P_{A_{k}}(t_{1},t_{2}), k\in E$  est la conjonction de formules atomiques définissant les items pour chaque attribut $A_k, k\in E$. 
...
Dans l'approche Sprex l'extraction d'une règle de préférence contextuelle $C\rightarrow X\succ Y$ à partir d'une paire de transactions $\left<t_1,t_2\right>$ se fait en comparant les items de $t_1$ et de $t_2$. Ainsi $C\subseteq t_1\cap t_2$, $X\subseteq t_1\mysetminus t_2$ et $Y\subseteq t_2\mysetminus t_2$. 
Dans notre approche l'extraction des règles de préférence contextuelles d'une paire de transaction $t_1,t_2$ se rapporte à extraire des formules de préférence de cette paire de transaction. Ceci se fait en comparaisant les items des transactions $t_1$ et $t_2$ et en inférant des formules logiques issues cette comparaison. Soit $\mathcal{F}$ l'ensemble des formules générées par la comparaisons entre les items de $t_1$  et ceux de $t_2$ (items de même type d'attribut). Soit $A_k$ un type d'attribut donné. \\
