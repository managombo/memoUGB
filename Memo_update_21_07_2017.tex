...
\begin{algorithm}[H]
	\Entree{l'ensemble des préférences de l'utilisateur $\mathcal{P}$, un seuil minimal de support $\delta$, un seuil minimal de confiance $\sigma$ et une fonction de modélisation $\pi$}
 	\Sortie{Modèle de règles de préférences $\mathcal{M}_\mathcal{P}$}
			...
			$\mathcal{M}=InterestingRules(\mathcal{P}_S,\delta,\sigma)$\;
			$\mathcal{M}_\mathcal{P}=\pi(sort(\mathcal{M}))$\;
			\textbf{Retourner} $\mathcal{M}_{\mathcal{P}}$;
 			}
...
L'algorithme que nous avons nommé ici \textit{InterrestingRules} est basé sur les travaux de l'article \cite{..} portant sur l'extraction de motifs minimaux à l'aide d'un algorithme qui a l'avantage d'être polynomial en espace. La consommation réduite de mémoire est un avantage dans notre approche vu que les itemsets obtenus sont de grande taille et engendrent l'usage de plus de mémoire dans l'extraction des règles de préférence interressantes que dans l'approche Sprex.\\

\textbf{Extraction de motifs minimaux}
\begin{defn}(Motif minimal)
Un motif est minimal pour une fonction $f$ et un langage \mathcal{L} si sa valeur pour $f$ est distincte de celle de chacun de ses sous ensembles.
\end{defn}
\begin{example}
\end{example}

\textit{InterestingRules} est un algorithme basé sur les travaux antérieurs de (Soulet et Al) et qui avait pour objectif l'extraction de motifs minimaux avec l'avantage d'être polynomial-delay et polynomial-space. En effet au cours du travail, l'utilisation initiale du programme $Sprex$ utilisant $Patterngrowth$ pour extraire les règles de préférences interressantes nous a confronté à un problème lié à la saturation mémoire. La nouvelle formulation créant des itemsets de préférences avec un plus grand nombre d'items que dans le cas initial des travaux de $Sprex$, celui ci bien qu'ayant pu être être utilisé avec succès dans le travail antéreur s'est avéré inadéquat dans notre cas. Nous nous sommes alors replié sur cette approche d'extraction de motifas fréquent qu'on va décrire dans l'algorithme \textit{InterrestingRules}.

Avant de définir l'algorithme InterestingRules, nous allons expliquer cette approche après avoir fourni des définitions de base.
